...
__attribute__((naked)) void task_scheduler() {
  ...
  if (cur_task == NULL) { // Initialisation
    ...
    asm volatile("pop %eax");
    asm volatile("pop %0":"=r"(cur_task->stack_r3));
    asm volatile("pop %eax");
    asm volatile("mov %%esp, %0":"=r"(cur_task->esp));
    asm volatile("mov %%ebp, %0":"=r"(cur_task->ebp));
    //cur_task->eip = *(uint32_t *)(cur_task->esp);
    cur_task = &tdt.task[cur_task->id%NUMBER_OF_TASKS + 1]; // Switch user1 to user2
    
    debug("Switch to user%d\n", cur_task->id);
    __set_ring3_segments();
    tss_addr->s0.esp = cur_task->esp;
    asm volatile("mov %0, %%esp"::"r"(cur_task->esp));
...
void load_task(int id, pde32_t *pgd, void *fct, uint32_t stack0, uint32_t stack3) {
  ...
  tdt.task[id].esp = stack0;
  tdt.task[id].stack_r3 = stack3;
  tdt.size++;
}
