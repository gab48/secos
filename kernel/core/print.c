...
static inline void __format_add_bin(buffer_t *buf, size_t len, uint64_t value,
                                    uint32_t n) {
  uint32_t i, bit;
...
static inline void __format_add_idec(buffer_t *buf, size_t len,
                                     sint64_t value) {
  char rep[24];
  buffer_t dec;

  if (!value) return __buf_add(buf, len, '0');
  ...
  while (dec.sz--) __buf_add(buf, len, dec.data.str[dec.sz]);
}

static inline void __format_add_udec(buffer_t *buf, size_t len,
                                     uint64_t value) {
  char rep[24];
  buffer_t dec;
...
size_t uint64_to_hex(buffer_t *buf, size_t len, uint64_t value,
                     size_t precision) {
  char rep[sizeof(uint64_t) * 2];
  size_t sz, rsz = 0;

  if (!precision || precision > 16) precision = -1;
  ...
  sz = rsz;
  while (rsz--) __buf_add(buf, len, rep[rsz]);
...
size_t __vsnprintf(char *buffer, size_t len, const char *format,
                   va_list params) {
  buffer_t buf;
  size_t size;
  char c;
